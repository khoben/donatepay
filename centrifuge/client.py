...
class Client:
    ...
    def __init__(self, address, credentials, **kwargs):
        ...
        self._recover_messages = kwargs.get("recover_messages", True)
        ...
        if success:
            if self._ping:
                self._ping_timer = self._loop.call_later(
                    self._ping_timeout, lambda: asyncio.ensure_future(
                        self._send_ping(), loop=self._loop)
                )
    ...
    def _subscribe(self, channels):
        ...
        for channel in channels:
            ...
            if channel in self._subs:
                params.update({
                    "recover": self._recover_messages,
